...
TEST(RateLimiterTest, testWrongParams)
{
  ...
  EXPECT_NO_THROW(
    control_toolbox::RateLimiter limiter(
      ...
      std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN(), -1.0,
      1.0));
  EXPECT_NO_THROW(
    control_toolbox::RateLimiter limiter(
      std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN(), -1.0, 1.0,
      std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN(), -1.0,
      1.0));
  EXPECT_NO_THROW(
    control_toolbox::RateLimiter limiter(
  ...
  EXPECT_THROW(
    control_toolbox::RateLimiter limiter(
      1.0, -1.0, -1.0, 1.0, std::numeric_limits<double>::quiet_NaN(),
  ...
  EXPECT_NO_THROW(
    control_toolbox::RateLimiter limiter(
      -1.0, 1.0, std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN(),
      std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN(), -1.0,
      1.0));
  EXPECT_NO_THROW(
    control_toolbox::RateLimiter limiter(
  ...
  EXPECT_NO_THROW(
    control_toolbox::RateLimiter limiter(
      -1.0, 1.0, -1.0, 1.0, -10.0, std::numeric_limits<double>::quiet_NaN(), -1.0, 1.0));
  EXPECT_NO_THROW(
    control_toolbox::RateLimiter limiter(
      -1.0, 1.0, -1.0, 1.0, std::numeric_limits<double>::quiet_NaN(), 10.0, -1.0, 1.0));
  EXPECT_THROW(
    control_toolbox::RateLimiter limiter(-1.0, 1.0, 1.0, -1.0, 10.0, -10.0, -1.0, 1.0),
    std::invalid_argument);
  ...
  EXPECT_NO_THROW(
    control_toolbox::RateLimiter limiter(
      -1.0, 1.0, -1.0, 1.0, std::numeric_limits<double>::quiet_NaN(),
      std::numeric_limits<double>::quiet_NaN(), -1.0, std::numeric_limits<double>::quiet_NaN()));
  EXPECT_NO_THROW(
    control_toolbox::RateLimiter limiter(
      -1.0, 1.0, -1.0, 1.0, std::numeric_limits<double>::quiet_NaN(),
...
{
  control_toolbox::RateLimiter limiter(
    -0.5, 1.0, -0.5, 1.0, std::numeric_limits<double>::quiet_NaN(),
    std::numeric_limits<double>::quiet_NaN(), -2.0, 5.0);
...
{
  {
    control_toolbox::RateLimiter limiter(
      ...
      std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN(), -0.5,
      5.0);
    double v = 10.0;
    double limiting_factor = limiter.limit_value(v);
    // check if the value is not limited
...

  {
    control_toolbox::RateLimiter limiter(
      std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN(), -0.5, 1.0,
      std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN(), -2.0,
      2.0);
    double v = 10.0;
    double limiting_factor = limiter.limit(v, 0.0, 0.0, 0.5);
    // first_derivative is now limiting, not value
...
{
  control_toolbox::RateLimiter limiter(-0.5, 1.0, -0.5, 1.0, -5.0, 10.0, -0.5, 5.0);
